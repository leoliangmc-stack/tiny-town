...
/** Runs the world through fake frames at the given speed until 23:59 of day 2. */
function runToLateEvening(speed: 1 | 100): World {
  ...
  while (world.time.totalTicks < ticksNeeded) {
    const ticks = Math.min(scheduler.ticksForFrame(1 / 60), ticksNeeded - world.time.totalTicks);
